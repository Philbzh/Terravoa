  ...
  return (
    ...
    >
      <Link href={`/regions/${region.slug}`} className="block">
        <div className={`relative overflow-hidden rounded-xl group ${featured ? 'h-[400px]' : 'h-[300px]'}`}>
          <Image
            src={region.imageSrc}
            alt={region.imageAlt}
            fill
          ...
          <div className="absolute bottom-8 left-8 z-10 max-w-[90%]">
            <h3 className="font-serif text-3xl text-white mb-1">{labels.name}</h3>
            <p className="text-white/80 text-sm font-sans uppercase tracking-[0.15em] mb-3">
              {labels.specialty}
            </p>
            <p className="text-white/60 text-sm font-sans max-w-md">{region.description}</p>
          </div>
